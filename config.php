...
if ($is_local) {
    ...
    $db_host = 'localhost';
    $db_name = 'bh_employment'; // Local database name
    $db_user = 'root'; // Local MySQL username
    $db_pass = ''; // Local MySQL password (usually empty for XAMPP/WAMP)
} else {
    // Production database settings
    $db_host = 'localhost';
    $db_name = 'bhemploy_admin';
    $db_user = 'bhemploy_root';
    $db_pass = 'Hl-&@13PiJXc';
}

// Create database connection lazily to reduce compute time and prevent "Too many connections"
class PDOProxy {
    private $pdo = null;
    private $dsn, $user, $pass, $options;
...
$pdo = new PDOProxy("mysql:host=$db_host;dbname=$db_name;charset=utf8", $db_user, $db_pass, [
    PDO::ATTR_PERSISTENT => false
]);

// Explicitly close connection on shutdown to prevent connection leaks
register_shutdown_function(function() use (&$pdo) {
    if ($pdo !== null && method_exists($pdo, 'close')) {
        $pdo->close();
    }
    $pdo = null;
});
...
if ($is_local) {
    $site_url = "http://localhost/bora-website/"; // Local development URL
} else {
    $site_url = "https://bhemployment.com/"; // Production URL - change to your actual domain
}
...
function isVerified() {
    ...
    // Check if the user is verified - handle various database output formats
    if (!isset($_SESSION['is_verified'])) {
        return false;
    }
    
    // Strict conversion to boolean to handle different server configurations
    return ($_SESSION['is_verified'] === true || $_SESSION['is_verified'] === 1 || $_SESSION['is_verified'] === '1');
}
