...
if (!isLoggedIn()) {
    flashMessage("You must be logged in to access this page", "danger");
    redirect('../login.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['application_id'], $_POST['status'])) {
    ...
    try {
        ...
        if ($stmt->rowCount() === 0) {
            ...
            redirect('my-job-applications.php');
        }
        ...
        $redirect_url = 'my-job-applications.php';
        if ($job_filter) {
...
<body>
                        ...
                        <div class="content-body">
                            ...
                            <div class="application-filters">
                                ...
                                <div class="status-tabs">
                                    <a href="my-job-applications.php" class="status-tab <?php echo (!$status_filter) ? 'active' : ''; ?>">
                                        All <span class="count"><?php echo $status_counts['all']; ?></span>
                                    </a>
                                    <a href="my-job-applications.php?status=pending<?php echo $job_filter ? '&job_id='.$job_filter : ''; ?>" class="status-tab <?php echo $status_filter === 'pending' ? 'active' : ''; ?>">
                                        Pending <span class="count"><?php echo $status_counts['pending']; ?></span>
                                    </a>
                                    <a href="my-job-applications.php?status=reviewed<?php echo $job_filter ? '&job_id='.$job_filter : ''; ?>" class="status-tab <?php echo $status_filter === 'reviewed' ? 'active' : ''; ?>">
                                        Reviewed <span class="count"><?php echo $status_counts['reviewed']; ?></span>
                                    </a>
                                    <a href="my-job-applications.php?status=interviewed<?php echo $job_filter ? '&job_id='.$job_filter : ''; ?>" class="status-tab <?php echo $status_filter === 'interviewed' ? 'active' : ''; ?>">
                                        Interviewed <span class="count"><?php echo $status_counts['interviewed']; ?></span>
                                    </a>
                                    <a href="my-job-applications.php?status=offered<?php echo $job_filter ? '&job_id='.$job_filter : ''; ?>" class="status-tab <?php echo $status_filter === 'offered' ? 'active' : ''; ?>">
                                        Offered <span class="count"><?php echo $status_counts['offered']; ?></span>
                                    </a>
                                    <a href="my-job-applications.php?status=rejected<?php echo $job_filter ? '&job_id='.$job_filter : ''; ?>" class="status-tab <?php echo $status_filter === 'rejected' ? 'active' : ''; ?>">
                            ...
                            <?php if (empty($applications)): ?>
                                <div class="empty-state">
                                    ...
                                    <?php if ($job_filter || $status_filter): ?>
                                        <a href="my-job-applications.php" class="btn-secondary">Clear Filters</a>
                                    <?php endif; ?>
                                </div>
                            <?php else: ?>
                                            ...
                                            <div class="application-content" id="application-content-<?php echo $application['id']; ?>">
                                                <div class="content-section">
                                                    <div class="application-resume">
                                                        ...
                                                        <?php if (!empty($application['resume_path']) && $application['resume_path'] !== 'Quick Apply'): ?>
                                                            <a href="../<?php echo htmlspecialchars($application['resume_path']); ?>" target="_blank" class="download-link">
                                                                <i class="fas fa-download"></i> Download Resume
                                                            </a>
                                                        <?php else: ?>
                                                            <div class="no-resume">
                                                                <i class="fas fa-exclamation-circle"></i> Quick Apply Application (No Resume File Attached)
                                                            </div>
                                                        <?php endif; ?>
                                                    </div>
                                                    ...
                                                    <?php if (!empty($application['cover_letter'])): ?>
                                                        <div class="application-cover-letter">
                                                            <h4><i class="fas fa-envelope-open-text"></i> Quick Apply Notes / Cover Letter</h4>
                                                ...
                                                <div class="content-section" style="padding-top: 0; margin-bottom: 0;">
                                                    <div class="status-update-section" style="flex: 1;">
                                                        <h4>Update Application Status</h4>
                                                        <form method="POST" action="my-job-applications.php<?php echo ($job_filter || $status_filter) ? '?' . http_build_query(array_filter(['job_id' => $job_filter, 'status' => $status_filter])) : ''; ?>">
                                                    ...
                                                    <div class="applicant-actions" style="flex: 1; display: flex; align-items: flex-end; justify-content: flex-end; padding-bottom: 15px;">
    ...
    <script>
        ...
        function applyFilters() {
            ...
            let url = 'my-job-applications.php';
            const params = [];
    ...
    .application-item.expanded .application-header {
        width: 300px;
        flex-direction: column;
    ...
    .application-item.expanded .application-meta {
        margin-top: 15px;
    }
    ...
    .application-item.expanded .application-actions {
        margin-left: 0;
        margin-top: 20px;
    }
    ...
    .application-item.expanded .application-content {
        display: block;
    }
