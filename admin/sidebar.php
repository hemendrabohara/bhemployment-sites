...
// Get count of pending candidate approvals
$pending_candidates_count = 0;
try {
    $candidates_stmt = $pdo->prepare("SELECT COUNT(*) as pending_count FROM candidates WHERE approval_status = 'pending'");
    $candidates_stmt->execute();
    $pending_candidates_count = $candidates_stmt->fetch()['pending_count'];
} catch (PDOException $e) {
    error_log("Error fetching pending candidates count: " . $e->getMessage());
}
?>
...
<ul class="dashboard-menu">
    ...
    <li class="<?php echo $current_page === 'manage-candidates.php' ? 'active' : ''; ?>">
        <a href="manage-candidates.php">
            <i class="fas fa-id-card"></i> Manage Candidates
            <?php if ($pending_candidates_count > 0): ?>
                <span class="badge-small"><?php echo $pending_candidates_count; ?></span>
            <?php endif; ?>
        </a>
    </li>
    ...
    <li class="<?php echo in_array($current_page, ['manage-blogs.php', 'add-blog.php', 'edit-blog.php']) ? 'active' : ''; ?>">
