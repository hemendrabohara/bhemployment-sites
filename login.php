        ...
        try {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                if (password_verify($password, $user['password'])) {
                    // Debug log to see verification status from database
                    $verification_data = [
                        'raw_value' => $user['is_verified'],
                        'type' => gettype($user['is_verified']),
                        'as_bool' => (bool)$user['is_verified'],
                        'as_int' => (int)$user['is_verified'],
                        'as_string' => (string)$user['is_verified']
                    ];
                    error_log("LOGIN DEBUG - Verification data: " . json_encode($verification_data));
                    ...
                    // Debug log to see how the session value was stored
                    error_log("LOGIN DEBUG - Session verification status: " . 
                             "type=" . gettype($_SESSION['is_verified']) . 
                             ", value=" . json_encode($_SESSION['is_verified']));
